...
String BrdNm          = "Nop-114"; //<-- ใส่ชือ RigName ตรงนี
...
const String FirmwareVer={"3.8"}; // Last of Version\
//Siwadol
...
//  if (String(eeprom_acct).equals(String(AcctName))){ //AcctName.length()
  if (eeprom_ssid.length()!=0){
    ...
    Serial.println("Change to new WiFi.");
    Serial.print("With : ");
    Serial.print(SSIDNm);
    Serial.print("/");
  ...
  Serial.println(" ");
  Serial.print("2-WiFi Log on with " );
  Serial.print(SSID);
  Serial.print("/");
...
//  Serial.println("------------------Get Account Name ------------------"); 
  ...
  for(int i=40;i< 60;i++){ 
    ...
    if(EEPROM.read(i) > 20)eeprom_ssid += char(EEPROM.read(i));
  }  
...
void SaveWiFi(){  
  ...
  for(int i=40;i < 60;i++)
  {
    ...
    Serial.println(SSIDNm[x]); 

    if(x < SSIDNm.length())EEPROM.write(i, SSIDNm[x]); //Write one by one with starting address of 0x0F
    x++;
  }
  SSID = SSIDNm;

  // เคลียร์พื้นที่ก่อน เขียน Password WiFi
  for(int i=60;i < 80;i++)
  {
   EEPROM.write(i, 0);
  }
...
//  Serial.println("------------------Show : BrdNm ------------------"); 
  ...
  {
    Serial.print(i);
    Serial.print(" SAVE PWD. ");
    Serial.println(Pwd[x]); 

    if(x < Pwd.length())EEPROM.write(i, Pwd[x]); //Write one by one with starting address of 0x0F
    x++;
  }
  PASSWORD = Pwd;
